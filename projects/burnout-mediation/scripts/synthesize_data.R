...
synthesize_data <- function(pattern_csv = "dataset_pattern.csv",
  ...
  local_range <- function(nm, default_lo, default_hi) {
  ...
  for (i in seq_len(nrow(pat))) {
    ...
    v <- switch(
      ...
      integer = {
        rng <- round(local_range(nm, 0, 100))
        sample(seq(rng[1], rng[2]), n, replace = TRUE)
      },
      numeric = {
        rng <- local_range(nm, 0, 100)
    ...
    miss_pct <- pat$missing_pct[i]
    if (!is.na(miss_pct) && miss_pct > 0 && type != "id_like") {
      n_miss <- min(n - 1, round(n * miss_pct / 100))
