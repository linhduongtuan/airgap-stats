...
# none of this existed before; the adjusted model used to ship with no check
# that its own assumptions held.
#
# Section 4 is Phase 2: a Firth penalized-likelihood refit (robust to the
# quasi-separation a modest sample with several covariates risks) and, for
...
# Optional: pin categorical reference levels for reproducibility (matches a
# paired R script's factor(x, levels = ...) calls). Leave commented out to
# let patsy/statsmodels pick each column's natural (alphabetical) reference
# level -- do this explicitly once you know your real data's category labels.
# dat["sex"]       = pd.Categorical(dat["sex"],       categories=["<reference>", "<other>"])
# dat["treatment"] = pd.Categorical(dat["treatment"], categories=["<reference>", "<other>"])
...
# statsmodels treats pd.Categorical with C() — use Q() so column names with
# spaces/reserved characters still parse as formula terms
def quote(v):
    return f'Q("{v}")'
...
# Auto-detect: numeric, non-binary members of predictor + covariates. Box-Tidwell
# needs strictly-positive continuous variables; anything else is reported
# `skipped`/`error` by box_tidwell_test itself rather than silently dropped here.
continuous_for_linearity = list(dict.fromkeys(
    v for v in [predictor_var, *covariate_vars]
    if v in dat.columns and pd.api.types.is_numeric_dtype(dat[v]) and dat[v].dropna().nunique() > 2
))
if not continuous_for_linearity:
    print("No numeric continuous predictor/covariates detected -- skipping Box-Tidwell.")
    bt_table = pd.DataFrame(columns=["variable", "status", "p_value", "detail"])
else:
...
print(
    "\nNOTE: on the delivered synthetic dataset, VIF/influence/linearity/bootstrap "
    "results are expected to look bland or degenerate -- columns are simulated "
...
flagged_vars = bt_table.loc[bt_table["status"] == "nonlinear_suspected", "variable"].tolist() if not bt_table.empty else []
if not flagged_vars:
